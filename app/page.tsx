...
export default function Home() {
  ...
  const [rankingMonth, setRankingMonth] = useState("Tháng 8, 2026");
  ...
  const isCheckinWindowOpen = new Date().getDay() === 3;
  ...
  useEffect(() => {
    if (!supabase) return;
    const client = supabase;
    const month = rankingMonth === "Tháng 8, 2026" ? "2026-08-01" : "2026-07-01";
    const loadRanking = async () => {
      ...
      setRankingRows(data.map((row: any, index: number) => {
  ...
  return <main className={"app-shell " + (sidebarOpen ? "sidebar-open" : "")}>
    <aside className="sidebar">
      ...
      <nav onClick={() => setSidebarOpen(false)}>
        ...
        <button className={screen === "ranking" ? "active" : ""} onClick={() => { setScreen("ranking"); setRankingMonth("Tháng 8, 2026"); }}><span>▥</span> Bảng xếp hạng</button>
        {screen === "ranking" && <div className="sidebar-subnav"><button className={rankingMonth === "Tháng 8, 2026" ? "chosen" : ""} onClick={() => setRankingMonth("Tháng 8, 2026")}>BXH hiện tại</button><button className={rankingMonth === "Tháng 7, 2026" ? "chosen" : ""} onClick={() => setRankingMonth("Tháng 7, 2026")}>Tháng 7, 2026</button></div>}
